...
export default class Core {
  ...
  async noopwait (_, { meta: { gearbox_id } }) {
    ...
    debug(`registered noopwait ${gearbox_id}, pausing`);
  ...
  async noopgo (id, { meta: { gearbox_id } }) {
    ...
    if (watch) {
      debug(`noopwait ${id} exists, unpausing`);
      watch.fn();
      this.nooping.delete(id);
      data = true;
    } else {
      debug(`noopwait ${id} didn’t exist, noop`);
    }
  ...
  async queue ({ name, args, ...job }) {
    ...
    if (job.dedupe) {
      debug(`checking running jobs for dedupe '${job.dedupe}'`);
      ...
      job.dedupe = uuid.v4();
      debug(`assigning dedupe: ${job.dedupe}`);
    }
  ...
  async jobData ({ id, data = null, status = null, progress = null, append = false }) {
    const job = await this.dbJobs.where({ id }).first();
    ...
    if (data !== undefined && data !== null) {
      ...
      if (append || typeof append === 'string') {
        ...
        debug(`data is to be appended`, { separator: JSON.stringify(append) });
        update.result_data = job.result_data + append + JSON.stringify(data);
      } else {
  ...
  async checkJob ({ id, job }) {
    ...
    assert(id);
    assert(job);
    debug(`checking over job ${id}: status = ${job.status}`);
    ...
    switch (job.status) {
      ...
      case 'waiting':
        if (
          ...
          const updated = await this.dbJobs.where({ id, status: 'waiting' }).update({
            status: 'ready',
            updated: this.db.raw('CURRENT_TIMESTAMP')
          });

          if (!updated) {
            debug(`job ${id} changed while we were marking, rechecking`);
            this.recheckJob(id);
            return;
          }
      ...
      case 'running':
        if (!await this.jobIsOnGearman(job)) {
          ...
          const updated = await this.dbJobs.where({ id, status: 'running' }).update({
            status: 'missing',
            updated: this.db.raw('CURRENT_TIMESTAMP')
          });
    ...
    if (
      ...
      const updated = await this.dbJobs.where({ id, status: 'ready' }).update({
        status: 'waiting',
        updated: this.db.raw('CURRENT_TIMESTAMP')
      });
    ...
    try {
      ...
      await this.rpc.request(job.method_name, args, {
        priority, dedupe, meta: { gearbox_id: job.id }
      });
...

      {
        ...
        if (updated) {
          ...
          debug(`job ${id} was changed while we were marking, rechecking`);
          this.recheckJob(id);
        }
      }
    } catch (err) {
  ...
  async jobIsOnGearman (job) {
    ...
    const hasUnique = (await this.gear.getuniquejobs()).includes(job.dedupe);
