...
export default class Core {
  ...
  async jobData ({ id, data = null, status = null, progress = null }) {
    const job = await this.db_jobs.where({ id });
    if (!job) throw new RpcError({ code: 404, message: 'no such job' });

    dcore(`got an update about a job: ${id}`);
    const update = {
      updated: this.db.raw('CURRENT_TIMESTAMP')
    };

    if (data !== undefined && data !== null) {
      dcore(`job ${id} receiving data`);
    ...
    if (progress !== undefined && progress !== null) {
      ...
      if (!isNaN(progress) && progress >= 0) {
        dcore(`job ${id} receiving progress: ${progress}`);
    ...
    if (status && ['running', 'almost-done'].includes(job.status)) {
      if (!['complete', 'errored'].includes(status)) {
        dcore(`job ${id} status was invalid: ${status}`);
        status = 'invalid';
      }
    ...
    if (update.status) dcore(`job ${id} marked as ${update.status}`);
    else dcore(`job ${id} updated from worker`);

    if (status === 'errored') {
      this.failJob(job).catch(console.error);
    } else if (status === 'complete') {
      this.checkSuccessors(job.id).catch(console.error);
    }
  }
  ...
  async checkJob ({ id, job }) {
    ...
    switch (job.status) {
      ...
      case 'running':
        if (!(await this.gear.getuniquejobs()).includes(job.disambiguator)) {
          dcore(`job ${id} was running but is no longer on gearman`);
          await this.db_jobs.where({ id }).update({
            status: 'missing',
            updated: this.db.raw('CURRENT_TIMESTAMP')
          });
          dcore(`job ${id} marked as missing`);
          this.waitUntil(job, job.updated + MISSING_TIMEOUT);
          return;
        }

        if (job.runner_instance !== INSTANCE_ID) {
          dcore(`job ${id} is running but it's no longer being watched`);
      ...
      case 'missing':
        if (((+job.updated) + MISSING_TIMEOUT) < Date.now()) {
          dcore(`job ${id} has been missing for ${ms(new Date - job.updated)}, assuming failed`);
          this.failJob(job, 'went missing');
        } else {
          dcore(`job ${id} is missing`);
          this.waitUntil(job, (+job.updated) + MISSING_TIMEOUT);
        }
        return;
    ...
    if (ambigu) {
      ...
      dcore(`job ${id} marked as duplicate`);
      return;
    }
    ...
    try {
      ...
      dcore(`job ${id} marked as almost-done`);
    } catch (err) {
  ...
  async failJob (job, err = null) {
    ...
    if (job.retries < job.max_retries) {
      ...
      dcore(`job ${id} has been retried ${job.retries} out of ${job.max_retries} times, bail`);
    }
  ...
  async checkSuccessors (jobid) {
    ...
    dcore(`job ${jobid} has successors: ${ids.join(', ')}`);
