...
export default class Core {
    ...
    switch (job.status) {
      ...
      case 'waiting':
        if (
          ...
          this.triggerWatchersForJob(id);
          break;
        } else if (job.prior_status === 'errored') {
          return this.failJob(job, new RpcError({ code: -28523, message: 'Dependency errored' }));
        } else {
  ...
  async checkSuccessors (jobid) {
    const successors = await this.dbJobs.where('after_id', jobid);
