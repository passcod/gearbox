...
export class Gearman {
  ...
  async request (method, params = {}, priority = null) {
    const pack = common.send(method, params, getId());
    const data = await this.gear.submitJob(pack.method, { priority }, JSON.stringify(pack));
    return checkResult(pack.id, checkData(JSON.parse(data)));
  }

  async notify (method, params = {}, priority = null) {
    const pack = common.send(method, params);
    await this.gear.submitJobBg(pack.method, { priority }, JSON.stringify(pack));
  }
